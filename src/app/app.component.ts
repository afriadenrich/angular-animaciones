import { animate, state, style, transition, trigger } from '@angular/animations'; // Importamos las funciones necesarias para animaciones
...
@Component({
  ...
  animations: [ // Definimos las animaciones utilizadas en este componente
    trigger('fadeIn', [ // Creamos la animación 'fadeIn'
    ...
    trigger('slideIn', [ // Creamos la animación 'slideIn'
    ...
    trigger('bounce', [ // Creamos la animación 'bounce'
      ...
      transition('normal => bounce', [animate('0.5s ease-in-out', style({ transform: 'translateY(-300px)' })), animate('0.5s ease-in-out')]), // Transición de 'normal' a 'bounce' con varias animaciones
      transition('bounce => normal', [animate('0.5s ease-in-out')]), // Transición de 'bounce' a 'normal' con duración de 0.5 segundos
    ]),
    trigger('rotate', [ // Creamos la animación 'rotate'
    ...
    trigger('flip', [ // Creamos la animación 'flip'
    ...
    trigger('carouselAnimation', [ // Creamos la animación 'carouselAnimation'
      transition('void => *', [ // Definimos la transición de 'void' a cualquier estado
      ...
      transition('* => void', [ // Definimos la transición de cualquier estado a 'void'
        animate('500ms ease-in', style({ opacity: 0, transform: 'scale(0.95)' })), // Animación a opacidad 0 y escala 0.95
      ])
    ]),
    trigger('collapse', [ // Creamos la animación 'collapse'
    ...
    trigger('flash', [ // Creamos la animación 'flash'
    ...
    trigger('jello', [ // Creamos la animación 'jello'
    ...
    trigger('zoom', [ // Creamos la animación 'zoom'
      state('normal', style({ transform: 'scale(1)' })), // Definimos el estado 'normal' con escala 1
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  images = [ // Lista de imágenes para el carrusel
  ...
  get currentImage() { // Método getter para obtener la imagen actual
  ...
  preloadImages() { // Método para pre-cargar las imágenes
    const imageElements = this.images.map(src => { // Mapea la lista de URLs a elementos de imagen
    ...
    Promise.all(imageElements.map(img => new Promise<void>((resolve) => {
      img.onload = () => resolve(); // Resuelve la promesa cuando la imagen se carga
    }))).then(() => { // Cuando todas las promesas se resuelven
      this.isLoading = false; // Cambia el estado de carga a false (oculta el loader)
    });
  }

  startCarousel() { // Método para iniciar el carrusel
    this.stopCarousel(); // Detiene el carrusel en caso de que ya esté corriendo
    this.carouselInterval = setInterval(() => { // Establece un intervalo para cambiar la imagen
      this.nextImage(); // Cambia a la siguiente imagen
    }, 200); // Cambia de imagen cada 2 segundos
  }

  stopCarousel() { // Método para detener el carrusel
    if (this.carouselInterval) { // Verifica si hay un intervalo activo
      clearInterval(this.carouselInterval); // Limpia el intervalo
      this.carouselInterval = null; // Reinicia la variable del intervalo
    }
  }

  nextImage() { // Método para avanzar a la siguiente imagen
    this.currentImageIndex = (this.currentImageIndex + 1) % this.images.length; // Incrementa el índice de la imagen actual
  }

  previousImage() { // Método para retroceder a la imagen anterior
    this.currentImageIndex = // Cambia el índice actual
      this.currentImageIndex === 0 ? this.images.length - 1 : this.currentImageIndex - 1; // Si está en la primera imagen, vuelve a la última
  }
